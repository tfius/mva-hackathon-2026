...
| In silico | PTC 746 nt upstream of the final exon–exon junction → NMD predicted; AlphaGenome splicing 0.087 (no splice effect) | AlphaMissense 0.923, MVP 0.852, REVEL 0.472; AlphaGenome splicing 0.048 (no splice effect); AlphaGenome AVI PHRED 25.6, **not counted** — 60 % of it is AlphaMissense re-entered as a feature |
...
**AlphaGenome Atlas pass** (`10_alphagenome_atlas.py`, `11_alphagenome_scan.py`). Every SNV call across the 120-gene panel (±50 kb, 22,236 calls) was annotated with the Atlas merged splicing score, looking specifically for the allele a consequence filter discards — a deep-intronic or synonymous cryptic-splice change. The top twenty splice-scoring calls were pushed through VEP. **All are common**: gnomAD genome AF 0.08–0.99, and the four that ClinVar has seen are Benign or Conflicting. The single rare one, a heterozygous `PKD1` intron +24 change (gnomAD exome AF 4 × 10⁻⁶, AlphaGenome splicing 1.18, SpliceAI 0), is in a gene unrelated to the phenotype and is recorded here as a predictor disagreement, not a finding. In the MVA genes themselves nothing scores above the panel's own 99th percentile except two common *BUB3* intronic polymorphisms (AF 0.14–0.19). The same file gives the Atlas variant-impact score (AVI) for both BUB1B alleles — PHRED 33.8 and 25.6 — but the SHAP decomposition shipped with the Atlas shows allele B's score is **60 % AlphaMissense, 36 % conservation, 4 % AlphaGenome tracks**, so AVI restates AlphaMissense rather than corroborating it and is not cited as evidence. The scan was then run **genome-wide** on all 3,981,890 called SNVs. 82 score at or above the Atlas's own 99.9th splicing percentile and 79 have AVI PHRED ≥ 30; after VEP, sixteen of those 140 are rare (gnomAD AF < 0.01 or absent). Every one is heterozygous, no gene carries two of them, and none is an MVA, spindle-checkpoint or chromosomal-instability gene. The eight rare splice-scoring calls (`HLA-DRB1`, `ATP6V1E1`, `HMCN2`, `PRPH`, `PLAAT2`, `TSPAN17`, `GAR1`, `RBKS`) are single heterozygous hits in genes that do not fit the phenotype; they are listed in the working directory, not here. One number from the same table is worth keeping: on AVI alone, allele A ranks 27th of four million calls, allele B **306th**. A pathogenicity score does not find allele B; the recessive pairing under a nominated gene does, which is the point §S3 makes. What the Atlas contributes that is new is the splice negative and a conservation signal (phastCons 470-way, Cactus 241-way) at both sites, and both alleles are confirmed as pure coding lesions: ATAC, DNase, ChIP, CAGE and RNA-seq contributions are all near zero.

**The splice negative is calibrated, and the gene's whole mutational surface is mapped.** Two further Atlas passes test the negative rather than asserting it. First, the model demonstrably works in this gene: the eight canonical-splice-site pathogenic records in *BUB1B* score 2.54–3.40 on the Atlas splicing scale, and two of the three pathogenic *intronic* records score 3.04 and 2.08 — against which alleles A and B, at 0.087 and 0.048, sit 37× and 68× below the median known splice-pathogenic variant at the same locus. Second, all three alternates at all 60,154 positions of the gene were scored — 180,462 changes — which locates **35 deep-intronic positions** where a single change would exceed the Atlas's genome-wide 99.9th percentile, and 38 exonic positions where a change would disrupt splicing rather than protein. **The proband carries no variant at any of the 35, and the nearest called variant is 606 bp away.** That replaces the ±20 bp canonical-window negative with a measured one. Detail, both figures and the base-editing screen are in [`track1-alphagenome-atlas.md`](track1-alphagenome-atlas.md).

**A calibration result that runs against the case, and is kept.** Scoring all 2,382 ClinVar SNVs in *BUB1B* separates the classified sets completely: every Benign/Likely benign record scores ≤ 23.78 and every Pathogenic/Likely pathogenic record ≥ 27.12. **Allele B scores 25.61 — inside the 3.3-point band where no classified variant sits.** The Atlas declines to classify it, exactly as ACMG does, and that is the correct behaviour for a **hypomorph**: MVA1 needs one null and one partially functional allele, so an allele B that scored with the pathogenic set would be evidence against the mechanism. Worth recording alongside it: across all consequence classes in these six genes, AlphaMissense on its own separates pathogenic from benign at **AUC 0.526** — no better than chance — because the pathogenic burden in *BUB1B* is truncating and a missense predictor scores truncating variants at zero.
...
- The Exomiser run is coding-focused. The AlphaGenome Atlas splicing pass, run panel-wide, genome-wide and across the whole *BUB1B* mutational surface, covers SNVs only — the five in-gene BUB1B indels and every indel panel-wide are unscored by any splice model here — and a REMM non-coding pass and a SHEPHERD knowledge-graph ranking are still to come; none is expected to change the call, but each would strengthen the claim that nothing else was missed.
- AlphaGenome's AVI score is **not** a second opinion on allele B. Its own feature attribution makes it 60 % AlphaMissense; it is reported for transparency and given no weight in the classification.
...
| **SpliceAI** | deep learning | Exomiser pathogenicity source; nothing surfaced at the locus | **Not run standalone**; the standalone splice negative comes from AlphaGenome instead |
| **AlphaGenome Atlas — merged splicing** | deep learning (DeepMind), precomputed SNV scores | 0.087 / 0.048 on alleles A / B; panel-wide and genome-wide cryptic-splice scans clean | The **splice negative** for every in-gene SNV; SNVs only, indels unscored. Non-commercial terms |
| **AlphaGenome Atlas — AVI** | linear ensemble over AlphaGenome tracks, AlphaMissense, conservation and termination flags | PHRED 33.8 / 25.6 | **Given no weight.** SHAP shows allele B's score is 60 % AlphaMissense; citing both would be double counting. Conservation contribution (phastCons, Cactus) noted. Permissive terms; SHAP file non-commercial |
...
Pipeline `mva/track1/00`–`11`, panels and Ensembl helpers in `mva/src/mva/`. Environment: bcftools 1.24 / htslib 1.23.1, Python 3.11, Ensembl VEP 116 (REST for locus work, offline cache for the genome-wide pass), ClinVar GRCh38 (August 2026), AlphaMissense hg38, Exomiser 15.1.0 + 2512 data, AlphaGenome Atlas SNV releases dated 27 August 2026 (AVI, splicing) and 1 September 2026 (feature importances), queried with tabix. The Atlas query for the two alleles is committed as `mva/results/texdata_alphagenome_bub1b.tsv`; the panel-wide scan carries proband genotypes and stays with the sequence data.
...
AlphaMissense is CC BY-NC-SA 4.0 and is used here under its non-commercial terms. The AlphaGenome Atlas AVI scores are released for commercial and non-commercial use; the Atlas splicing scores and feature importances are non-commercial only and are used under those terms. ClinVar, gnomAD, Ensembl and UniProt are used under their respective open licences. This report and the accompanying code are released CC BY 4.0.
